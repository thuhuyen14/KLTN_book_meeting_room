...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  const rooms = await fetchData('/api/analytic/rooms');
  renderChart(
    'roomChart',
    'bar',
    rooms.map(r => r.name),
  ...
  const days = await fetchData('/api/analytic/days');
  renderChart(
    'dayChart',
    'line',
  ...
  const bookingHours = await fetchData('/api/analytic/rooms/hours');
  renderChart(
    'hourChart',
    'bar',
  ...
  const users = await fetchData('/api/analytic/users');
  renderChart(
    'userChart',
    'bar',
  ...
  const docStatus = await fetchData('/api/analytic/docs/status');
  renderChart(
    'signStatusChart',
    'pie',
  ...
  const signerStats = await fetchData('/api/analytic/docs/signers');
  renderChart(
    'signerChart',
    'bar',
  ...
  const signDays = await fetchData('/api/analytic/docs/days');
  renderChart(
    'signDayChart',
    'line',
  ...
  const overview = await fetchData('/api/analytic/overview');
